...
export function PushedTodaySection() {
  ...
  const pushed = useMemo(
    () => (items ?? []).filter((i) => i.status === 'skipped' || i.status === 'deferred'),
    [items],
  );
  ...
  const { data: tasksMap = {} } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from('tasks')
        .select('id, deferred_until, category')
        .in('id', taskIds);
  ...
  return (
    <div className="rounded-[14px] bg-card" style={{ border: '0.5px solid rgba(0,0,0,0.04)' }}>
      ...
      >
        <span className="text-[13px] font-medium" style={{ color: '#5C3D1E' }}>
          ...
          {pushed.map((item) => (
            ...
            >
              <div className="flex-1 min-w-0">
                <p className="text-[14px] truncate" style={{ color: '#5C3D1E' }}>{item.title}</p>
                <p className="text-[12px]" style={{ color: '#8B7355' }}>{subtitleFor(item)}</p>
              </div>
              <button
                onClick={() => bringBack(item)}
                disabled={busyId === item.id}
                className="px-3 rounded-lg text-[12px] font-medium border h-8 flex-shrink-0 disabled:opacity-50"
                style={{ borderColor: '#8B7355', color: '#5C3D1E' }}
              >
